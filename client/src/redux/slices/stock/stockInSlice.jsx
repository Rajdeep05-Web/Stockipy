...
const API_URL =
  ...
  "/api/v1/stock-ins";
const API_URL_FILE = "/api/v1/file";
  ...
  async (stockInDataAll, { rejectWithValue }) => {
    ...
    try {
      ...
      if (file) {
        ...
        ? await API.post(API_URL_FILE, formData, {
      ...
      const { data } = await API.post(`${API_URL}`, stockInData);
      return data;
    } catch (error) {
      return rejectWithValue(error?.response?.data.error || error.message);
    }
...
export const updateStockIn = createAsyncThunk(
  ...
  async (stockInUpdateData, { rejectWithValue }) => {
    ...
    try {
      if(file){
        ...
        if (fileUploadResponse) {
          ...
          if(fileCloudUrl != ""){
            ...
            await API.delete(`${API_URL_FILE}/${public_id}`);
          }
          stockInData.fileCloudUrl = fileUploadResponse.data.fileUrl;
        } 
      } else {
      ...
      const { data } = await API.put(`${API_URL}/${id}`, stockInData);
      return data;
    } catch (error) {
      console.log(error);
      return rejectWithValue(error?.response?.data.error || error.message);
...
export const deleteStockIn = createAsyncThunk(
  ...
  async ({ id }, { rejectWithValue }) => {
    try {
      const { deleteedData } = await API.delete(`${API_URL}/${id}`);
      return deleteedData;
    } catch (error) {
      return rejectWithValue(error?.response?.data.error || error.message);
    }
